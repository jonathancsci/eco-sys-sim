...
class Animal:
    ...
    def init_scores(self, grid: Grid):
        self.preferences = dict.fromkeys(
            grid.neighbors, self.energy / 2 + self.dice_roll()
        )
    ...
    def score_grid(self, grid):
        for o in grid.occupants:
            ...
            if type(o) in self.fears:
                ...
                if (
                    self in grid.occupants
                    and self.fights_back
        ...
        if self.fights_back:
            for o in grid.occupants:
                if o.alive and (type(o) in self.fears):
                    return AttackAction(self, o.size * 0.3 - self.size * 0.1, grid, o)
        return None
        ...
        if not self.is_full():
            for o in grid.occupants:
                if o.alive and (type(o) in self.attacks):
                    return AttackAction(self, o.size * 0.2 - self.size * 0.1, grid, o)
        return None
